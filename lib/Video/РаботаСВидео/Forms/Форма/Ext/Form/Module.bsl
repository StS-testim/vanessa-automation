...
Функция СоздатьЗаголовокСценария(ПараметрыВидео, ИмяСценария, НомерСценария)
	...
	СценарийMP3 = ПолучитьФайлMP3ИзТекста(ПараметрыВидео, "" + СловоСценарий + ". " + Текст, "scen_" + ДобавитьНулей(НомерСценария, 5) + "_begin");
	
	Текст   = "" + СловоСценарий + "\n\n " + ИмяСценария;
	
	ФайлВидео   = СоздатьФайлВидеоИзТекста(ПараметрыВидео, Текст, СценарийMP3, "scen_" + ДобавитьНулей(НомерСценария, 5) + "_begin");
...
Процедура СобратьВидеоВставкуИзТекста(ПараметрыВидео, ИмяФайлаШага, Знач ТекстВставки, Знач ТекстДиктора, НомерСценария, НомерШага, ПараметрыГолоса, Шаг)
	...
	Если ПараметрыВидео.ЗаписьВидеоКэшироватьВидеоВставки Тогда
		ДобавитьВидеоВставкуШагаВКэш(ИмяФайлаШага, ИсходныйТекст, ПараметрыВидео, Шаг); //+++ ЦВ_ПР 03.07.2025 Добавлен параметр Шаг
	КонецЕсли;	 
КонецПроцедуры

&НаКлиенте 
//+++ ЦВ_ПР 03.07.2025 В штатную процедуру добавлен параметр Шаг
Процедура СобратьВидеоВставкуИзКартинки(Шаг, ПараметрыВидео, ИмяФайлаШага, Знач ТекстВставки, ИмяКартинки, НомерСценария, НомерШага, ПараметрыГолоса, ДопПараметры = Неопределено)
	ИмяФайлаШага = СтрЗаменить(ИмяФайлаШага, ".mp4", ".mpg");// так реально создан mpg и дальше нам надо mpg
	Ванесса.УдалитьФайлыКомандаСистемы(ИмяФайлаШага);
	ИсходныйТекст = ТекстВставки;
	//+++ ЦВ_ПР 03.07.2025 Если картинки разные, а текст одинаковый (в частности - пустой), то текст в Кэш берет неверно. Далее замена ТекстВставки => ТекстВставкиКэш  
	ТекстВставкиКэш = ТекстВставки + "/" + ИмяКартинки;  
	//--- ЦВ_ПР 03.07.2025 конец вставки
	
	
	Если ПараметрыВидео.ЗаписьВидеоКэшироватьВидеоВставки Тогда
		Если ПараметрыВидео.ДанныеКэшВидеоВставки[ТекстВставкиКэш] <> Неопределено Тогда //+++ ЦВ_ПР 03.07.2025 Замена ТекстВставки => ТекстВставкиКэш
			КаталогГдеЛежитКэшВидеоВставок = КаталогГдеЛежитКэш(ПараметрыВидео, ПараметрыВидео.ЗаписьВидеоКэшВидеоВставок);
			ФайлВидео = КаталогГдеЛежитКэшВидеоВставок + "\" + ПараметрыВидео.ДанныеКэшВидеоВставки[ТекстВставкиКэш];      //+++ ЦВ_ПР 03.07.2025 Замена ТекстВставки => ТекстВставкиКэш
			Ванесса.КопироватьФайлКомандаСистемы(ФайлВидео, ИмяФайлаШага);	
			Возврат;
		КонецЕсли;	 
	КонецЕсли;	 
...
Процедура СобратьФинальноеВидеоИзЧастей(ПараметрыВидео, МассивЧастей)
	...
	Если ПараметрыВидео.Свойство("ПолныйПутьКФиче") Тогда
		ФичаФайл = Новый Файл(ПараметрыВидео.ПолныйПутьКФиче);
	Иначе
		ФичаФайл = Новый Файл("result.feature");
	КонецЕсли;	
...
Функция ЭтоШагПервыйВВеткеДерева(ПараметрыВидео, Шаг, МассивВставок, Сценарий)
	Если Шаг.Свойство("ВнутриГруппыКотораяРаботаетКакОдинШаг") Тогда
		...
		КонецЕсли;	 
	КонецЕсли;	 
	//+++ ЦВ_ПР 20.06.2025 Если сказано Игнорировать, то значит исключаем тоже 
	...
	Если Индекс = 0 Тогда     
		//+++ ЦВ_ПР 20.06.2025 Строка Начала Цикла и аналогичных по списку не подходит для видеовставки 
		Если РодительШага.Свойство("ДопТип") Тогда
			Если РодительШага.ДопТип = "Цикл" или РодительШага.ДопТип = "Условие" или РодительШага.ДопТип = "УсловиеИначе" или РодительШага.ДопТип = "УсловиеИначеЕсли" 
				или РодительШага.ДопТип = "Попытка" или РодительШага.ДопТип = "Исключение" Тогда  
			Возврат Ложь;
		КонецЕсли;	 
 		КонецЕсли;
		//--- ЦВ_ПР 20.06.2025 	
	...
	Если Шаг.Свойство("ВнутриГруппыКотораяРаботаетКакОдинШаг") Тогда
		Если Шаг.ВнутриГруппыКотораяРаботаетКакОдинШаг Тогда
			Если НЕ Шаг.ПервыйВГруппеШагов Тогда
				// Значит это шаг, который находится в группе, которая объединена в один шаг
				Возврат Истина;
			КонецЕсли;	 
		КонецЕсли;	 
	КонецЕсли;	 
...
Процедура ЗавершитьЗаписьИнструкцииВидео(ПараметрыВидео)
	...
	Для каждого Сценарий Из МассивСценариевДляВыполнения Цикл
		...
		ПараметрыВидео.Вставить("ПолныйПутьКФиче", Сценарий.ПолныйПутьКФиче);
		...
		//+++ ЦВ_ПР 20.06.2025 Добавлено, т.к. ошибка для последующих сценариев в случае, когда принудительно шагом остановлен текущий сценарий
		...
		Для каждого Шаг Из Шаги Цикл
			...
			Если НЕ Шаг.Свойство("ТекстИнструкция") Тогда
				Шаг.Вставить("ТекстИнструкция", "");
			КонецЕсли;	
			...
			Если Шаг.Свойство("ИгнорироватьШагВАвтоинструкции") Тогда
				...
				КонецЕсли;	 
			КонецЕсли;	 
			//+++ ЦВ_ПР 11.07.2025 Исключим явно шаги текстовой инструкции
			...
			Если ЭтоШагБезСкриншота Тогда
				Продолжить;
			КонецЕсли;	 
  			//--- ЦВ_ПР 11.07.2025 конец вставки
			...
			//Так было ИмяФайлаШага = ПолучитьИмяФайлаВидеоШага(ПараметрыВидео);
			//+++ ЦВ_ПР 19.06.2025 Для озвучки в Цикле пришлось сделать уникальное имя с Универсальным временем. Заменим имя файла, если есть его описание и значение в Шаге
			Если Шаг.Свойство("ИмяВидеоФайла") и ЗначениеЗаполнено(Шаг.ИмяВидеоФайла) тогда 
				ИмяФайлаШага = Шаг.ИмяВидеоФайла;	
			иначе     
				//ЦВ_ПР Вероятно этот шаг не записан, поэтому игнорируем его
				ИмяФайлаШага = Неопределено;
				Продолжить;
			...
			//+++ ЦВ_ПР 20.06.2025 Файлы в \TTS\ заберем как было сохранено в Шаг.ИмяАудиоФайла при записи 
			//ИмяMp3Шага = СтрЗаменить(ИмяMp3Шага, Сред(ИмяMp3Шага, 19,15), "");                                                 
			Если Шаг.Свойство("ИмяАудиоФайла") тогда
				ИмяMp3Шага = Шаг.ИмяАудиоФайла;
			КонецЕсли;	
			//--- ЦВ_ПР 20.06.2025
			...
			ИначеЕсли Ванесса.ЭтоШагВидеовставкиКартинки(Шаг) Тогда
				...
				КонецЕсли;	 
				//+++ ЦВ_ПР 27.06.2025 Корректировки для исправления ошибок вставки картинок в цикле
 				ИмяФайлаШагаВставки = СтрЗаменить(ИмяФайлаШага, "_main.", "_ins.");
				ТекстДиктора = Шаг.ЗначенияПараметров[1].Значение.Значение;
				ИмяФайлаКартинки = Шаг.ЗначенияПараметров[0].Значение.Значение; 
                СобратьВидеоВставкуИзКартинки(Шаг, ПараметрыВидео, ИмяФайлаШагаВставки, 
					ТекстДиктора, // текст диктора  +++ ЦВ_ПР 27.06.2025 Замена Шаг.ЗначенияПараметров[1].Значение.Значение на переменную ТекстДиктора , т.к. может содержать переменные
					ИмяФайлаКартинки, // имя картинки   +++ ЦВ_ПР 27.06.2025 Замена Шаг.ЗначенияПараметров[0].Значение.Значение на переменную ИмяФайлаКартинки  , т.к. может содержать переменные
					НомерСценария, НомерШага, ПараметрыГолоса, ДопПараметрыСборкиИзКартинки);  
				ФайлЧасти = Новый Файл(ИмяФайлаШагаВставки);
				МассивЧастей.Добавить(ФайлЧасти);
				//--- ЦВ_ПР 03.07.2025 Конец вставки
			...
			Иначе
				...
				ИначеЕсли ДлительностьВидео > (ДлительностьMP3 + 1) И (НЕ Шаг.Свойство("МасштабироватьВВидео") ИЛИ Шаг.МасштабироватьВВидео) Тогда
...
Процедура ЗавершитьЗаписьИнструкцииHTML(ПараметрыВидео)
	...
	Для каждого Сценарий Из МассивСценариевДляВыполнения Цикл
		...
		Для каждого Шаг Из Шаги Цикл
			...
			//ИмяФайлаШага = ПолучитьИмяФайлаВидеоШага(ПараметрыВидео);
			//+++ ЦВ_ПР 19.06.2025 Для озвучки в Цикле пришлось сделать уникальное имя с Универсальным временем. Заменим имя файла, если есть его значение описание в Шаге
			Если Шаг.Свойство("ИмяВидеоФайла") и ЗначениеЗаполнено(Шаг.ИмяВидеоФайла) тогда 
				ИмяФайлаШага = Шаг.ИмяВидеоФайла;	
			иначе   
				//ЦВ_ПР Вероятно этот шаг не записан, поэтому игнорируем его
			...
			Или  СнипетыСубтитры.Найти(СнипетНрег) <> Неопределено
				...
				НомерШага = НомерШага + 1;
				Стр = "<p><b>%1</b>. %2</p>";     
				//+++ ЦВ_ПР 03.07.2025 Вероятно и здесь нужно поменять ,3 на 5 для номера Шага (меняли ранее для имени файлов с целью корректной сортировки)
				Стр = СтрЗаменить(Стр, "%1", Ванесса.ПолучитьФорматированныйНомер(НомерШага, 5));
...
Функция СБЕР_ТокенАвторизации(ПараметрыВидео)
	...
	Заголовки.Вставить("Authorization", "Basic " + ПараметрыВидео.ЗаписьВидеоСберTTSClientSecret);
	Заголовки.Вставить("RqUID", 		ПараметрыВидео.ЗаписьВидеоСберTTSClientID);
	...
	СберСрокТокена = МестноеВремя('19700101' + Цел(Данные["expires_at"]/1000));
	...
	Для каждого Сценарий Из МассивСценариевДляВыполнения Цикл
		...
		Для каждого Шаг Из Шаги Цикл
			...
			Если НЕ Шаг.Свойство("ТекстИнструкция") Тогда
				Шаг.Вставить("ТекстИнструкция", "");
			КонецЕсли;	
			//+++ ЦВ_ПР 20.06.2025 Для решения вопросов озвучки шагов в цикле требуется сохранить имя с привязкой к "обновленному" шагу
			Если НЕ Шаг.Свойство("ИмяАудиоФайла") Тогда
				Шаг.Вставить("ИмяАудиоФайла", "");
			КонецЕсли;
			//--- ЦВ_ПР 20.06.2025			
			...
			ПараметрыГолоса = ПолучитьПараметрыГолоса(ПараметрыВидео, Шаг);
			Если Шаг.Свойство("МассивТеговШага") И Шаг.МассивТеговШага <> Неопределено И Шаг.МассивТеговШага.Количество() > 0 Тогда
				ПараметрыГолоса.Вставить("МассивТеговШага", Шаг.МассивТеговШага);
			КонецЕсли;		
			//+++ ЦВ_ПР 27.05.2025 номер шага добавление лидирующих нулей до 5-ти знаков (было 3 знака после "_step_" ) [проблема сортировки]
			ИмяФайла = "scen_" + ДобавитьНулей(НомерСценария, 5) + "_step_" + ДобавитьНулей(НомерШага, 5) + "_main";
			...
			КонецЕсли;
	       	//+++ ЦВ_ПР 20.06.2025 Запомним обновленное ИмяАудиоФайла
			ИмяMP3Запомнить = СтрЗаменить(ИмяMP3,ПараметрыВидео.ЗаписьВидеоКаталогДляВременныхФайлов + "\TTS\", "");
			Шаг.Вставить("ИмяАудиоФайла",ИмяMP3Запомнить); 
			МассивСценариевДляВыполнения[НомерСценария].Шаги[НомерШага].Вставить("ИмяАудиоФайла", ИмяMP3Запомнить); 
			ПараметрыВидео.Вставить("Шаг", Шаг); 
			//--- ЦВ_ПР 20.06.2025
...
Функция ПолучитьПараметрыГолоса(ПараметрыВидео, Шаг)
	...
	Если Шаг.Свойство("МассивТеговШага") Тогда
		МассивТеговШага = Шаг.МассивТеговШага;
	Иначе
		МассивТеговШага = Новый Массив;
	КонецЕсли;	
	ОбработатьТегиУправляющиеГолосом(МассивТеговШага, ПараметрыГолоса);
...
&НаКлиенте
Процедура ДобавитьВидеоВставкуШагаВКэш(ИмяФайлаШага, Текст, ПараметрыВидео, Шаг)
	...
	//+++ ЦВ_ПР 03.07.2025 Если картинки разные, а текст одинаковый (в частности - пустой), то текст в Кэш берет неверно  
	ТекстКэш = Текст;
	Если Ванесса.ЭтоШагВидеовставкиКартинки(Шаг) тогда  
		ТекстДиктора = Шаг.ЗначенияПараметров[1].Значение.Значение;
		ИмяФайлаКартинки = Шаг.ЗначенияПараметров[0].Значение.Значение; 
		ТекстКэш = ТекстДиктора + "/" + ИмяФайлаКартинки; 
	КонецЕсли;	
    //--- ЦВ_ПР 03.07.2025 конец вставки
	ДанныеКэшВидеоВставки.Вставить(ТекстКэш, ИмяФайла); //+++ ЦВ_ПР 03.07.2025 Замена Текст => ТекстКэш  
КонецПроцедуры 
...
Функция ПолучитьИмяФайлаВидеоШага(ПараметрыВидео)
    //+++ ЦВ_ПР 18.06.2025  Формирование имени в цикле приводит к замещению файлов. Необходимо добавить дополнительную уникальность +Формат(ТекущаяУниверсальнаяДатаВМиллисекундах(), "ЧГ=; ЧН=0")
	Если ПараметрыВидео.ЭтоИнициализация Тогда
		Стр = "init.mp4";
	Иначе	
		//+++ ЦВ_ПР  было без дополнения универсальной даты. Универсальная дата используется для уникальности видео (Шаги в цикле), а также их сортировки в хронологии)
		// Так было Стр = "scen_" + ДобавитьНулей(ПараметрыВидео.ТекИД_СценарияВМассиве, 3) + "_step_" 
		//	+ ДобавитьНулей(ПараметрыВидео.ТекИД_ШагаВМассиве, 4) + "_main.mp4"; 
		Стр = "scen_" + ДобавитьНулей(ПараметрыВидео.ТекИД_СценарияВМассиве, 3) + "_step_" 
			+ Формат(ТекущаяУниверсальнаяДатаВМиллисекундах() , "ЧГ=; ЧН=0") + "_" + ДобавитьНулей(ПараметрыВидео.ТекИД_ШагаВМассиве, 4) +  "_main.mp4";
	КонецЕсли;	 
...
Процедура ОстановитьЗаписьВидеоОдногоШага(ПараметрыВидео) Экспорт
	...
	Пока Истина Цикл
		...
		Если (КолПопыток >= МаксКолПопыток) И (ТекущаяДата() >= (ТекДат + МаксКолПопыток)) Тогда
			ПолучилосьЗакрытьVLC = Истина;
			Прервать;
		КонецЕсли;	 
		
	КонецЦикла;	
	
	Ванесса.ЗаписатьЛогВЖРИнформация("ОстановитьЗаписьВидеоОдногоШага", "ПолучилосьЗакрытьVLC=" + ПолучилосьЗакрытьVLC);
