	...
	Для каждого Сценарий Из МассивСценариевДляВыполнения Цикл
		...
		Для каждого Шаг Из Шаги Цикл
			...
			Или  СнипетыСубтитры.Найти(СнипетНрег) <> Неопределено
				...
				НомерШага = НомерШага + 1;
				Стр = "<p><b>%1</b>. %2</p>";     
				//+++ ЦВ_ПР 03.07.2025 Вероятно и здесь нужно поменять ,3 на 4 для номера Шага (меняли ранее для имени файлов с целью корректной сортировки)
				Стр = СтрЗаменить(Стр, "%1", Ванесса.ПолучитьФорматированныйНомер(НомерШага, 4));
...
Процедура СоздатьФайлыTTS(ПараметрыВидео) Экспорт
	...
	Для каждого Сценарий Из МассивСценариевДляВыполнения Цикл
		...
		Для каждого Шаг Из Шаги Цикл
			...
			КонецЕсли;		
			//+++ ЦВ_ПР 27.05.2025 номер шага добавление лидирующих нулей до 4-х знаков (было 3 знака после "_step_" ) [проблема сортировки]
			ИмяФайла = "scen_" + ДобавитьНулей(НомерСценария, 3) + "_step_" + ДобавитьНулей(НомерШага, 4) + "_main";
