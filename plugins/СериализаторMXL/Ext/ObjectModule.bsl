...
Функция СоздатьДанныеПоМакетам(ТестОбъект, ИменаМакетов, ВозвращатьНаборЗаписей = Ложь) Экспорт
	...
	КонецЦикла;
	Возврат СоздатьДанныеПоДокументуМакета(МассивМакетов, ВозвращатьНаборЗаписей);
КонецФункции
...
Функция СоздатьДанныеПоДокументуМакета(ТабличныйДокументИлиМассивТабличныхДокументов, РежимыЗагрузкиИлиИмяКолонкиЗамещения = Неопределено, ИмяКолонкиЗамещения = Неопределено, ВозвращатьНаборЗаписей = Ложь) Экспорт
	...
	Для каждого Элем Из Данные Цикл
		...
		ИначеЕсли Найти(Стр, ЗначениеСоответствияРегистрБухгалтерии) > 0 Тогда
			...
			КонецЕсли;			
		КонецЕсли;
		// с не сериализуемыми типами макет на клиент не возвращаем
		Если ЗначениеЗаполнено(ИмяРегистра) И ВозвращатьНаборЗаписей И (Метаданные.РегистрыБухгалтерии.Содержит(Элем.Значение.Метаданные())
			или Метаданные.РегистрыНакопления.Содержит(Элем.Значение.Метаданные())
			или Метаданные.РегистрыРасчета.Содержит(Элем.Значение.Метаданные())
			или Метаданные.РегистрыСведений.Содержит(Элем.Значение.Метаданные())) Тогда
			ВозвращатьНаборЗаписей = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция СоздатьДанныеПоТабличномуДокументу(ТабличныйДокумент, РежимыЗагрузки, ИмяКолонкиЗамещения, ВозвращатьДанные) Экспорт
	// для обратной совместимости
	Данные = СоздатьДанныеПоДокументуМакета(ТабличныйДокумент, РежимыЗагрузки, ИмяКолонкиЗамещения, ВозвращатьДанные);
	Возврат Данные; 
КонецФункции	
...
Функция ПрочитатьДанныеИзТекстовогоДокумента(ПротоДанные,ТекстовыйДокумент,ИмяКолонкиЗамещения)
	...
	Для Каждого ОбъектJSON ИЗ JSON Цикл	
		...
		// предопределенные обрабатываются отдельно
		Если ПротоОбъект.Режимы.Предопределенный Тогда
			ПротоОбъект.ИмяПредопределенного = ПротоОбъект.Значение;
			// приводим к одинаковому заполнению с табличным документом
			Если ПротоОбъект.Свойство("Значение") Тогда
				ПротоОбъект.Удалить("Значение");
			КонецЕсли;
		Конецесли;
...
Функция ВычислитьЗначениеПоТипуПоля(НаборМдРеквизитов,стрПоле,ВызыватьОшибкуНеЗаполненногоЗначения = Ложь)
	Значение = Неопределено;
	...
	Попытка
		...
		Иначе
			...
			Если ЗначениеПоУмолчанию = Неопределено И ТипЗначения.Типы().Количество() > 1 И НЕ УжеВычислилиЗначение И стрПоле.Значение <> "00000000-0000-0000-0000-000000000000" Тогда
				// у субконто составной тип
				// поиск по первому типу приводит к выдаче исключения
				Для Каждого Тип Из ТипЗначения.Типы() Цикл
					стрПоле.ДополнительныйТип = Новый Массив;
					стрПоле.ДополнительныйТип.Добавить(Тип);
					Значение = ВычислитьЗначениеПоТипуПоля(НаборМдРеквизитов, стрПоле, Ложь);
					Если Найти(Строка(Значение), "Объект не найден") = 0 И ЗначениеЗаполнено(Значение) Тогда
						Прервать; 
					КонецЕсли;
				КонецЦикла;
				// вызовем исключение если по всем типам не найдено значение
				Если НЕ ЗначениеЗаполнено(Значение) Тогда
					ТекстИсключения = "Не нашли реквизит """ + стрПоле.Значение + """ по режиму """ + стрПоле.Режим + """";
		...
		Если НЕ УжеВычислилиЗначение и ЕстьМетаданныеРеквизита Тогда
			...
			Если ВызыватьОшибкуНеЗаполненногоЗначения И ЗначениеЗаполнено(стрПоле.Значение) И НЕ ЗначениеЗаполнено(Значение) Тогда //Если НЕ ЗначениеЗаполнено(Значение) Тогда
				ТекстСообщения = ПолучитьТекстСообщенияПользователю("ЗначениеНеНайдено: Реквизит <%1>: Искали значение <%2>, но не нашли.");
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",стрПоле.Имя);
				ТекстСообщения = СтрЗаменить(ТекстСообщения,"%2",стрПоле.Значение);
				ВызватьИсключение ТекстСообщения;
...
Функция ПолучитьРезультатЗапросаСКД(СКД)
	...
	КомпоновщикМакета = Новый КомпоновщикМакетаКомпоновкиДанных;
	МакетКомпоновки = КомпоновщикМакета.Выполнить(СКД, Компоновщик.ПолучитьНастройки(),,, Тип("ГенераторМакетаКомпоновкиДанныхДляКоллекцииЗначений"));
	// TODO: разобраться почему в типовой УТ при выгрузке РСВ "НастройкиУчетаМаркируемойПродукцииИСМП" набор данных в макете пустой при наличии в СКД
	Если МакетКомпоновки.НаборыДанных.Количество() = 0 Тогда
		Запрос = Новый Запрос(СКД.НаборыДанных[0].Запрос);
	Иначе
		Запрос = Новый Запрос(МакетКомпоновки.НаборыДанных.ОсновнойЗапрос.Запрос);
	КонецЕсли;
	ПараметрыЗапроса = Запрос.НайтиПараметры();
			...
			Если КорневойТипКонфигурации = "РегистрБухгалтерии" Тогда
				...
				ИначеЕсли НЕ РеквизитМд.Балансовый Тогда
					...
					Для Каждого Суффикс Из Суффиксы Цикл
						Если ОписательОбъекта.Свойство(имяРеквизита + Суффикс) Тогда
						...
						Иначе
							...
							ИмитаторРеквизитаМд = Новый Структура("Имя, Тип", имяРеквизита, РеквизитМд.Тип);
							ДобавилиРеквизит = ДобавитьРеквизитОбъектаВВыгрузку(ДанныеДляВыгрузки, МассивДанных, Объект, ИмитаторРеквизитаМд, ЗначениеРеквизита, Мд, ШапкаДанных); 							
							Прервать;
						КонецЕсли;
					КонецЦикла;
...
Функция ДобавитьРеквизитОбъектаВВыгрузку(ДанныеДляВыгрузки, МассивДанных, Объект, РеквизитМд, ЗначениеРеквизита, Мд, ШапкаДанных)
	...
	// для предопределнных элементов нет смысла выгружать ссылку
	// она может отличаться в базах разработки и тестирования
	Если Объект.Предопределенный И ЗаписьВыгрузки.Реквизит = "Ссылка" Тогда
		Возврат Истина;
	КонецЕсли;
