...
@IgnoreOn8314
@IgnoreOn8315
Сценарий: Прикрепление скриншота, когда используется VanessaExt. Все сеансы 1С. Все окна клиента тестирования. Плюс дополнительные процессы.
	...
	* Добавлям снятие скриншотов с дополнительных процессов
		И я завершаю выполнение процесса ОС "notepad.exe"
		И Пауза 2
		И я выполняю код встроенного языка
		"""bsl
			ЗапуститьПриложение("notepad.exe")
		"""
		И Пауза 1
		И я жду появления процесса ОС "notepad.exe" в течение 30 секунд
		И в поле с именем 'ПроцессыДляСнятияСкриншотов' я ввожу текст "notepad.exe"
	...
	Тогда выражение внутреннего языка '$НашлиСкриншот$ >= 5' Истинно
	...
	Тогда выражение внутреннего языка '$ДанныеJson$.attachments.Количество() >= 5' Истинно
	Тогда выражение внутреннего языка '$ДанныеJson$.steps[6].attachments.Количество() >= 5' Истинно
...
Сценарий: Прикрепление скриншота, когда используется VanessaExt. Одно окно. Текущее окно клиента тестирования. Плюс дополнительные процессы.
	Дано Я закрыл все окна клиентского приложения
	И я закрываю сеанс TESTCLIENT
	Когда я запускаю служебный сеанс TestClient с ключом TestManager в той же базе
	...
	* Добавлям снятие скриншотов с дополнительных процессов
	...
	Тогда выражение внутреннего языка '$ДанныеJson$.steps[1].attachments.Количество() >= 2' Истинно
