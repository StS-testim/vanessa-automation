...
Сценарий: Основной сценарий проверки работы шагов Gherkin из поставляемой библиотеки и проверка генерации шагов.
	...
	И текущее время в диапазоне от "00:00:00" до "23:59:59"
	И Я запоминаю значение выражения 'Формат(ТекущаяДата() + 5, "ДФ=HH:mm:ss")' в переменную "ТекДатаПлюс5Секунд"
	Тогда я жду наступления времени "$ТекДатаПлюс5Секунд$"
	...
	Если файл "c:\temp\ТестКопированияКаталога" существует тогда
		Тогда я очищаю каталог "c:\temp\ТестКопированияКаталога"
	Тогда я копирую каталог файлов "$КаталогФичи$\files" "c:\temp\ТестКопированияКаталога"
	И Файл "C:\Temp\ТестКопированияКаталога\ТестовыйКаталог\Тестовый файл.txt" существует
	И Файл "C:\Temp\ТестКопированияКаталога\ФайлПроверки1.txt" существует
	...
	Если версия платформы ">=" "8.3.19" Тогда
		И я активизирую процесс по заголовку "*Конфигурация*"
	...
	//Проверка https://github.com/Pr-Mex/vanessa-automation/issues/2237
	* Если форма списка в режиме "Иерархический список", то корректно работает проверка пустых групп
		...
		И в поле с именем 'Наименование' я ввожу текст "Группа1"
		И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
		И я жду закрытия окна "Справочник2 (создание группы) *" в течение 20 секунд
		Тогда открылось окно "Справочник2"
		И я нажимаю на кнопку с именем '*СоздатьГруппу'
		Тогда открылось окно "Справочник2 (создание группы)"
		И в поле с именем 'Наименование' я ввожу текст "Группа2"
		И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
		И я жду закрытия окна "Справочник2 (создание группы) *" в течение 20 секунд
...
#Область Создание элементов шапки	
	...
	И для каждой строки макета "ПроверкаЦиклаПоМакету" я выполняю
	...
	Иначе	
		Тогда переменная "КолОкон" имеет значение 3
	

	И я активизирую форму "Справочник1"
	Тогда открылось окно "Справочник1"
	И я жду, что активным окном станет "Справочник1" в течение 3 секунд	
	...
	Если версия платформы ">=" "8.3.16" Тогда
		...
		Иначе	
			...
			И Пауза 1
			И в таблице "Список" в дополнение формы с именем 'СписокСтрокаПоиска' я ввожу текст 'Тестовый Элемент 1'
			И Пауза 1
			Тогда в таблице "Список" количество строк "равно" 1
	...
	Если версия платформы ">=" "8.3.16" Тогда
		...
		Иначе	
			Тогда в таблице "Список" количество строк "равно" 2
			И я активизирую дополнение формы с именем "СписокСтрокаПоиска"
			И Пауза 1
			И в дополнение формы с именем 'СписокСтрокаПоиска' я ввожу текст 'Тестовый Элемент 1'
			И Пауза 1
			Тогда в таблице "Список" количество строк "равно" 1
			И я нажимаю кнопку очистить у дополнения формы с именем "СписокСтрокаПоиска"
			И Пауза 1
			И я отменяю редактирование у дополнения формы с именем "СписокСтрокаПоиска"
			И Пауза 1
	...
	//содержит строки только из макета
	И таблица "Список" содержит строки только из макета "СписокСтрокиФайл1"

	//Проверка когда имя колонки в макете указано с использованием *
	И таблица "Список" содержит строки только из макета "СписокСтрокиФайл3"
	И Я запоминаю значение выражения '0' в переменную "БылаОшибкаСписокСтрокиФайл2"
	Попытка
		И таблица "Список" содержит строки только из макета "СписокСтрокиФайл2"
	Исключение
		И я запоминаю текст ошибки в переменную "ТекстОшибки"
		И Я запоминаю значение выражения '1' в переменную "БылаОшибкаСписокСтрокиФайл2"
		
	Тогда переменная "БылаОшибкаСписокСтрокиФайл2" имеет значение 1
	Тогда выражение внутреннего языка 'Найти(Контекст.ТекстОшибки, "Обнаружено расхождение между таблицой клиента тестирования и эталоном") > 0' Истинно

	//содержит строки только из макета по шаблону
	И таблица "Список" содержит строки только из макета "СписокСтрокиФайлПоШаблону1" по шаблону
	И Я запоминаю значение выражения '0' в переменную "БылаОшибкаСписокСтрокиФайлПоШаблону2"
	Попытка
		И таблица "Список" содержит строки только из макета "СписокСтрокиФайлПоШаблону2" по шаблону
	Исключение
		И я запоминаю текст ошибки в переменную "ТекстОшибки"
		И Я запоминаю значение выражения '1' в переменную "БылаОшибкаСписокСтрокиФайлПоШаблону2"
		
	Тогда переменная "БылаОшибкаСписокСтрокиФайлПоШаблону2" имеет значение 1
	Тогда выражение внутреннего языка 'Найти(Контекст.ТекстОшибки, "Обнаружено расхождение между таблицой клиента тестирования и эталоном") > 0' Истинно

	И я запоминаю заголовок поля с именем "Код" как "ЗаголовокКолонкиКод"

	//содержит строки только из таблицы Gherkin
	И таблица "Список" содержит только указанные строки:
		| 'Наименование'       | '$ЗаголовокКолонкиКод$' |
		| 'Тестовый Элемент 1' | '000000001'             |
		| 'Тестовый Элемент 2' | '000000002'             |
	И Я запоминаю значение выражения '0' в переменную "БылаОшибкаСписокСтрокиФайл1"
	Попытка
		И таблица "Список" содержит только указанные строки:
			| 'Наименование'       | 'Код'       |
			| 'Тестовый Элемент 2' | '000000002' |
	Исключение
		И я запоминаю текст ошибки в переменную "ТекстОшибки"
		И Я запоминаю значение выражения '1' в переменную "БылаОшибкаСписокСтрокиФайл1"
		
	Тогда переменная "БылаОшибкаСписокСтрокиФайл1" имеет значение 1
	Тогда выражение внутреннего языка 'Найти(Контекст.ТекстОшибки, "Обнаружено расхождение между таблицой клиента тестирования и эталоном") > 0' Истинно


	//содержит строки только из таблицы Gherkin по шаблону
	И таблица "Список" содержит только указанные строки по шаблону:
		| 'Наименование'       | 'Код'       |
		| 'Тестовый Элемент 1' | '000*1'     |
		| 'Тестовый Элемент 2' | '000000002' |
	И Я запоминаю значение выражения '0' в переменную "БылаОшибкаСписокСтрокиФайл2"
	Попытка
		И таблица "Список" содержит только указанные строки по шаблону:
			| 'Наименование'       | 'Код'       |
			| 'Тестовый Элемент 2' | '0000*02'   |
	Исключение
		И я запоминаю текст ошибки в переменную "ТекстОшибки"
		И Я запоминаю значение выражения '1' в переменную "БылаОшибкаСписокСтрокиФайл2"
	...
	И я запоминаю представление текущего поля как "ТекЗначениеЯчейки"
	И переменная "ТекЗначениеЯчейки" имеет значение "Тестовый Элемент 1"
	...
	Затем в течение 100 секунд я выполняю
		Дано я нажимаю на кнопку с именем '*Обновить'
	...
	//Проверка когда в колонка указана с использованием *
	И в таблице "Список" я перехожу к строке:
		| 'Наименовани*'       |
		| 'Тестовый Элемент 1' |
	
	И в таблице "Список" поле с именем "Наименование" имеет значение "Тестовый Элемент*" по шаблону
	...
	И я запоминаю значение таблицы "Список" как "Таб3"
	И я запоминаю значение таблицы "Список" как "Таб4"
	И таблица "Таб3" содержится в таблице "Таб4"


	И Я запоминаю значение выражения 'Ванесса.Объект.КоличествоПопытокВыполненияДействия' в переменную "КоличествоПопытокВыполненияДействия"
	И я устанавливаю настройку Vanessa Automation "КоличествоПопытокВыполненияДействия" 2
	

	// Проверка перехода только вниз
	И в таблице "Список" я перехожу к первой строке
	И в таблице "Список" я перехожу к строке направление вниз
		| 'Наименование'       |
		| 'Тестовый Элемент 2' |

	И Я запоминаю значение выражения '0' в переменную "ФлагОшибкаПереходаКСтроке"
	Попытка
			И в таблице "Список" я перехожу к строке направление вниз
				| 'Наименование'       |
				| 'Тестовый Элемент 1' |
	Исключение
			И Я запоминаю значение выражения '1' в переменную "ФлагОшибкаПереходаКСтроке"

	Тогда переменная "ФлагОшибкаПереходаКСтроке" имеет значение 1 
		



	// Проверка перехода только вверх
	И в таблице "Список" я перехожу к последней строке
	И в таблице "Список" я перехожу к строке направление вверх
		| 'Наименование'       |
		| 'Тестовый Элемент 1' |
	...
	И я устанавливаю настройку Vanessa Automation "КоличествоПопытокВыполненияДействия" "$КоличествоПопытокВыполненияДействия$"
	...
	И реквизиты формы имеют значение:
		| 'Наименование'  | 'Тестовый Элемент 2' |
		| 'Наименование1' | 'Тестовый Элемент 2' |
		| 'Наименование2' | 'Тестовый Элемент 2' |

	И реквизиты формы имеют значение по шаблону
		| 'Наименование'  | 'Тестовый Элемент *' |
		| 'Наименование1' | 'Тестовый Элемент *' |
		| 'Наименование2' | 'Тестовый Элемент *' |
...
#	И в открытой форме я открываю выпадающий список с заголовком "Реквизит1"
	...
	И Я закрываю окно 'Тестовый Элемент*(Справочник1)*'
	И я запоминаю текст предупреждения в переменную "ТекстОкна"
	Тогда переменная "ТекстОкна" имеет значение "Данные были изменены. Сохранить изменения?"
	...
	Если Версия платформы ">=" "8.3.12" Тогда
		...
		Иначе
			...
			Иначе	
				И я меняю значение переключателя с именем 'РеквизитПереключательВысотаСтроки2' на 'Первое значение'
	...
	Если Версия платформы ">=" "8.3.12" Тогда
		...
		И у элемента формы с именем 'Наименование' текст редактирования стал равен "Тестовый Элемент 2" в течение 10 секунд
		И у элемента формы с именем 'Наименование' текст редактирования стал равен "Тестовый Элемент*" по шаблону в течение 10 секунд
	...
	Если версия платформы ">=" "8.3.25" Тогда
		Если это интерфейс85 (расширение) Тогда
			И в таблице 'ТабличнаяЧасть1' я нажимаю на кнопку с именем 'ТабличнаяЧасть1ПанельДействийВыделенныхСтрокСнятьВыделение'

		И в таблице 'ТабличнаяЧасть1' я копирую строку
		И в таблице 'ТабличнаяЧасть1' я завершаю редактирование строки
		И я жду, что в таблице 'ТабличнаяЧасть1' количество строк будет "равно" 2 в течение 20 секунд
		И в таблице "ТабличнаяЧасть1" я перехожу к последней строке
		И Пауза 0.1
	...
	И я запоминаю значение таблицы "ТабличнаяЧасть1" как "ТоварыЭталон"
		| 'ЧастьЗаголовка1\nЧастьЗаголовка2'|
	...
	Если это интерфейс85 (расширение) Тогда
		И в таблице 'ТабличнаяЧасть1' я завершаю редактирование строки
		И в таблице 'ТабличнаяЧасть1' я выделяю текущую строку
			
	Если в таблице "ТабличнаяЧасть1" поле "Реквизит строка" имеет значение "" тогда
	...
	Если установлен режим совместимости "НеИспользовать" Тогда
			...
			Если Версия платформы ">=" "8.3.25" Тогда
					И Я очищаю окно сообщений пользователю
					И у поля "Надпись форматированная строка" я нажимаю гиперссылку 'Ссылк*1' по шаблону
					И В последнем сообщении TestClient есть строка "НавигационнаяСсылкаФорматированнойСтроки=Ссылка1"

					И Я очищаю окно сообщений пользователю
					И у поля "Надпись форматированная строка" я нажимаю гиперссылку 'Ссылк*2' по шаблону
					И В последнем сообщении TestClient есть строка "НавигационнаяСсылкаФорматированнойСтроки=Ссылка2"
	...
	Если версия платформы ">=" "8.3.25" Тогда
		//Работа с полем HTML документа
		И в поле с именем 'HTMLДокументНаФорме' я ввожу текст 
			"""
			<html dir="ltr"><head><base href="e1c://filev/G/Bases/8.3.25/">
			<meta http-equiv="content-type" content="text/html;charset=utf-8">
			</head><body>
			<font color="#413003">Текст до ссылок</font>
			<br><a id="Справочник2-1a7210f0-f286-11ee-9a04-4cedfb43b6d1" href="111">111</a>
			<br><a id="Справочник2-1a7210f1-f286-11ee-9a04-4cedfb43b6d1" href="222">222</a>
			<br><font color="#413003">Текст после ссылок</font><br></body></html>
			"""
		И Пауза 1
		Тогда HTML документ с именем "HTMLДокументНаФорме" стал равен
		"""
			<html dir="ltr"><head><base href="e1c://filev/G/Bases/8.3.25/">
			<meta http-equiv="content-type" content="text/html;charset=utf-8">
			</head><body>
			...
			<br><font color="#413003">Текст после ссылок</font><br></body></html>
		"""
		Тогда HTML документ с именем "HTMLДокументНаФорме" стал равен по шаблону
		"""
			<html dir="ltr"><head><base href="e1c://filev/G/Bases/*/">
			<meta http-equiv="content-type" content="text/html;charset=utf-8">
			</head><body>
			<font color="#413003">Текст до ссылок</font>
			<br><a id="Справочник2-1a7210f0-f286-11ee-9a04-4cedfb43b6d1" href="111">111</a>
			...
			<br><font color="#413003">Текст после ссылок</font><br></body></html>
		"""
		И в поле с именем 'ОтветHTML' я ввожу текст ""
		И в поле HTML документа с именем "HTMLДокументНаФорме" я нажимаю гиперссылку "111"
		Тогда элемент формы с именем 'ОтветHTML' стал равен "111"
		
		//Проверка свойства только просмотр и доступности у формы. Появилось только в 8.3.25
	...
	Если это интерфейс85 (расширение) Тогда
		Тогда открылось окно "Тестовый Элемент 2 (Справочник1)"
	Иначе
	...
	Если Версия платформы ">=" "8.3.10" Тогда
			...
			Тогда в логе сообщений TestClient есть строки по шаблону:
				| 'ГиперссылкаТолькоПросмотрНажатие*' |
				| 'ГиперссылкаТолькоПросмотр*'        |
...
#Область Проверка значений элементов формы
	...
	//Проверка когда в колонка указана с использованием *
	И     таблица формы с именем "ТабличнаяЧасть1" стала равной:
		| 'N' | 'Реквизит числ*' | 'Реквизит число' | 'Реквизит строка' | 'Реквизит дата' | 'Реквизит справочник выбор из формы списка' | 'Реквизит справочник' | 'Реквизит булево' |
		| '1' | '56,78'          |  '56,78'         | 'ТестСтрока'      | '11.09.1983'    | 'ТестовыйЭлемент21'                         | 'ТестовыйЭлемент21'   | 'Да'              |
	...
	//Проверка когда в колонка указана с использованием *
	И     таблица формы с именем "ТабличнаяЧасть1" содержит строки:
		| 'N' | 'Реквизит числ*' | 'Реквизит строка' | 'Реквизит дата' | 'Реквизит справочник выбор из формы списка' | 'Реквизит справочник' | 'Реквизит булево' |
		| '1' | '56,78'          | 'ТестСтрока'      | '11.09.1983'    | 'ТестовыйЭлемент21'                         | 'ТестовыйЭлемент21'   | 'Да'              |	
	...
	И я запоминаю строку "Да" в переменную "ПеремДа"
