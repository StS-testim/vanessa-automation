...
Процедура ВыполнитьЗапускОднойСборки(ПараметрыСборки)
	...
	Если ПараметрыСборки["ЗапускатьWatcher"] Тогда
		Лог.Информация("Устанавливаю настройки для Watcher");
		УстановитьНастройкиДляWatcher_ТекущаяСборка(ПараметрыСборки["ПутьКФайлуНастроекWatcher"], ИмяФайлаЛога);
	КонецЕсли;
...
Функция НачальныеНастройкиWatcher()
	
	Структура = Новый Структура;
	Структура.Вставить("ТаймаутНаИзменениеФайлаВСекундах", 1200);
	Структура.Вставить("ФайлЛога", "C:\Temp\WatcherLog.txt");
	Структура.Вставить("ФайлЗаКоторымНадоСледить", "");
	Структура.Вставить("СледитьЗаФайлом", Ложь);
	Структура.Вставить("ЗавершитьРаботу", Ложь);
	Структура.Вставить("ЧастотаПроверкиДатыИзмененияФайлаВСекундах", 30);
	
	Возврат Структура;
	
КонецФункции	

Процедура ЗаписатьФайлНастроекWatcher(ПутьКФайлуНастроекWatcher, НастройкиWatcher)
	
	СтрокаJson = ЗаписатьОбъектJSON(НастройкиWatcher);
	
	Для Сч = 1 По 10 Цикл
		Попытка
			ЗТ = Новый ЗаписьТекста(ПутьКФайлуНастроекWatcher,"UTF-8",, Ложь); 
...
Процедура ЗапуститьWatcher(ОсновнойФайлПараметров, ПутьКФайлуНастроекWatcher, НастройкиWatcher)
	Файл = Новый Файл(ОсновнойФайлПараметров);
	КаталогJSON = Файл.Путь;
	Файл = Новый Файл(КаталогJSON);
	КаталогTools = Файл.Путь;
	...
	ЗаписатьФайлНастроекWatcher(ПутьКФайлуНастроекWatcher, НастройкиWatcher);
	
	ТекстКоманды = "oscript " + КаталогСкриптов + "Watcher.os " + ПутьКФайлуНастроекWatcher;
	
	retCode = -1;
	Сообщить("Запуск Watcher");
	Сообщить(ТекстКоманды);
	ЗапуститьПриложение(ТекстКоманды,,Ложь,retCode);
КонецПроцедуры

Процедура УстановитьНастройкиДляWatcher_ТекущаяСборка(ПутьКФайлуНастроекWatcher, ИмяФайлаЛога)

	НастройкиWatcher = НачальныеНастройкиWatcher();
	НастройкиWatcher.Вставить("СледитьЗаФайлом", Истина);
	
	Если ЭтоLinux() Тогда
		НастройкиWatcher.Вставить("ФайлЗаКоторымНадоСледить", ИмяФайлаЛога);
	Иначе	
		НастройкиWatcher.Вставить("ФайлЗаКоторымНадоСледить", СтрЗаменить(ИмяФайлаЛога, "/", "\"));
	КонецЕсли;	
...
Процедура ОстановитьРаботуWatcher(ПутьКФайлуНастроекWatcher)
	НастройкиWatcher = НачальныеНастройкиWatcher();
	НастройкиWatcher.Вставить("СледитьЗаФайлом", Ложь);
	НастройкиWatcher.Вставить("ЗавершитьРаботу", Истина);
...
Процедура ВыполнитьЗапускВсехСборок(ОсновнойФайлПараметров, ОчищатьКеш = Ложь)
	...
	ЗапускатьWatcher = Ложь;
	ПутьКФайлуНастроекWatcher = "C:\temp\watcher.json";
	Если ОсновныеПараметры["ЗапускатьWatcher"] = Истина Тогда
		ЗапускатьWatcher = Истина;
	КонецЕсли;
	
	Если ЗапускатьWatcher Тогда
		НастройкиWatcher = НачальныеНастройкиWatcher();
		ЗапуститьWatcher(ОсновнойФайлПараметров, ПутьКФайлуНастроекWatcher, НастройкиWatcher);
	КонецЕсли;
	...
	Для каждого Сборка Из Сборки Цикл
		...
		ПараметрыСборки.Вставить("ЗапускатьWatcher", ЗапускатьWatcher);
		ПараметрыСборки.Вставить("ПутьКФайлуНастроекWatcher", ПутьКФайлуНастроекWatcher);
	...
	Если ЗапускатьWatcher Тогда
		Лог.Информация("Останавливаю Watcher");
		ОстановитьРаботуWatcher(ПутьКФайлуНастроекWatcher);
	КонецЕсли;
